...
package com.ceci.chess.lib.pgn;

import com.ceci.chess.lib.game.*;
import com.ceci.chess.lib.util.LargeFile;
import com.ceci.chess.lib.util.StringUtil;
...
public class PgnHolder {
     ...
     */
    public PgnHolder(String data, boolean isFile) {
        if (isFile)
        	setFileName(data);
        else this.data = data;
        setLazyLoad(false);
    ...
    public void loadPgn() throws Exception {
        ...
        LargeFile file = null;
        if (getFileName() != null)
        	lines = file = new LargeFile(getFileName());
        else
        {
        ...
        try {
            for (String line : lines) {
                try {
                    ...
                    throw new PgnException("Error parsing PGN[" + r + ", " + name + "]: "+line, e);
                }
